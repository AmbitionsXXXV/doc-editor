import { Range, getMarkRange, posToDOMRect } from '@tiptap/core'
...
class LinkToolbarView implements PluginView {
	...
	constructor(
		private readonly editor: EtcDocEditor<any, any, any>,
			...
			for (const mark of marksAtPos) {
				if (mark.type.name === this.pmView.state.schema.mark('link').type.name) {
					...
					this.keyboardHoveredLinkMarkRange =
						getMarkRange(this.pmView.state.selection.$from, mark.type, mark.attrs) ||
						undefined
