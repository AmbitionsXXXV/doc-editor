...
export const TableBlockContent = createStronglyTypedTiptapNode({
	...
	// default node view. `EtcDocTableView` extends it by wrapping it in a
	// `blockContent` element, so the DOM structure is consistent with other block
	// types.
	addNodeView() {
		return ({ node, HTMLAttributes }) => {
			class EtcDocTableView extends TableView {
				constructor(
					public node: PMNode,
			...
			return new EtcDocTableView(node, EMPTY_CELL_WIDTH, {
				...(this.options.domAttributes?.blockContent || {}),
				...HTMLAttributes,
			})
		}
