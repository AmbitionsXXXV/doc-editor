...
> = {
	render: (
		...
		/**
		 * The EtcDoc editor instance
		 * This is typed generically. If you want an editor with your custom schema, you need to
		 * cast it manually, e.g.: `const e = editor as EtcDocEditor<typeof mySchema>;`
		 */
		editor: EtcDocEditor<BlockSchemaWithBlock<T['type'], T>, I, S>,
...
>(blockConfig: T, blockImplementation: CustomBlockImplementation<T, I, S>) {
	const node = createStronglyTypedTiptapNode({
		...
		addNodeView() {
			return ({ getPos }) => {
				// Gets the EtcDoc editor instance
	...
	if (node.name !== blockConfig.type) {
		throw new Error('Node name does not match block type. This is a bug in EtcDoc.')
	}
