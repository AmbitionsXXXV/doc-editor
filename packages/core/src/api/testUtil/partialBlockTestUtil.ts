...
function partialContentToInlineContent(
	...
	if (Array.isArray(content)) {
		return content.flatMap((partialContent) => {
			...
			if (isPartialLinkInlineContent(partialContent)) {
				return {
					...partialContent,
					content: textShorthandToStyledText(partialContent.content),
				}
			}
			if (isStyledTextInlineContent(partialContent)) {
...
export function partialBlockToBlockForTesting<
	...
	I extends InlineContentSchema,
	S extends StyleSchema,
>(schema: BSchema, partialBlock: PartialBlock<BSchema, I, S>): Block<BSchema, I, S> {
	...
	if (contentType === 'inline') {
		...
		withDefaults.content = {
			type: 'tableContent',
			columnWidths:
				content?.columnWidths || content?.rows[0]?.cells.map(() => undefined) || [],
			rows:
