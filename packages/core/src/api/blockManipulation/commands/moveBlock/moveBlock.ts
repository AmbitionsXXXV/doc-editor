...
function getBlockSelectionData(
	...
	if (editor._tiptapEditor.state.selection instanceof CellSelection) {
		return {
			...selectionData,
			type: 'cell' as const,
			anchorCell: (editor._tiptapEditor.state.selection as CellSelection).$anchorCell
				.pos,
			headCell: (editor._tiptapEditor.state.selection as CellSelection).$headCell
				.pos,
		}
	}
	if (editor._tiptapEditor.state.selection instanceof NodeSelection) {
		return {
			...selectionData,
			type: 'node' as const,
...
export function moveBlockUp(editor: EtcDocEditor<any, any, any>) {
...
export function moveBlockDown(editor: EtcDocEditor<any, any, any>) {
	// This function currently only supports moving a single block.
	const editorSelection = editor.getSelection()
	if (editorSelection && editorSelection.blocks.length > 1) {
		return
