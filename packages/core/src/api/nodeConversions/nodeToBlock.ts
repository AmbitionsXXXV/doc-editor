...
/**
 * Converts an internal (prosemirror) table node contentto a EtcDoc Tablecontent
 */
...
/**
 * Converts an internal (prosemirror) content node to a EtcDoc InlineContent array.
 */
...
>(contentNode: Node, inlineContentSchema: I, styleSchema: S) {
	...
	// while in EtcDoc links are a type of inline content
...
/**
 * Convert a TipTap node to a EtcDoc block.
 */
...
>(
	...
	if (node.type.name !== 'blockContainer') {
		throw Error(
			`Node must be of type blockContainer, but is of type${node.type.name}.`,
		)
	}
	...
	const { blockContainer, blockContent, blockGroup } = getBlockInfoWithManualOffset(
		node,
		0,
	)
	...
	blockGroup?.node.forEach((child) => {
		children.push(
			nodeToBlock(child, blockSchema, inlineContentSchema, styleSchema, blockCache),
		)
	})
