...
>(props: {
	...
	const editor = useEtcDocEditor<BlockSchema, I, S>()

	const [menuContainerRef, setMenuContainerRef] = useState<HTMLDivElement | null>(
		null,
	)

	if (!editor.tableHandles) {
		throw new Error(
			'TableHandlesController can only be used when EtcDoc editor schema contains table block',
		)
	}
